...
try:
    from veritas import selfhosted_url as VERITAS_SELFHOSTED_URL
    from veritas import selfhosted_key as VERITAS_SELFHOSTED_KEY
except ImportError:
    ...
    def _create_client(self, api_key: str | None) -> OpenAI:
        ...
        if not self.base_url:
            raise ValueError(
                ...
                "  1. Add selfhosted_url to config/globals.yaml\n"
    ...
    def check_claim(
        ...
        try:
            ...
            while True:
                ...
                print(f"    [DEBUG] finish_reason={choice.finish_reason}, tool_calls={bool(message.tool_calls)}, content_len={len(message.content or '')}")
