...
def plot_stages(rows: list[Row], q_keys: list[tuple[int, int]]) -> list[tuple[str, Any]]:
    ...
    # Build category list: all stage labels sorted numerically + Dismissed at the end
    ...
    stage_categories = stage_numeric + ["Dismissed"]
    ...
    stage_zoom_categories = stage_desc + ["Dismissed"]
    figs.append(
        build_single_chart(
            "stages_zoomed",
            q_keys,
